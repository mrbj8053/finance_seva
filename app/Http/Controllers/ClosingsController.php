...
class ClosingsController extends Controller
{
    ...
    {
       $closings=DB::select("select i.user_id,u.own_id,u.name,(select sum(amount) from incomes where income_type='Direct' and cast(i.created_at as Date)='$date' and user_id=i.user_id) as Direct,
       ...
       (select sum(amount) from incomes where income_type='Reward' and cast(i.created_at as Date)='$date' and user_id=i.user_id) as Reward from incomes as i join users as u on i.user_id=u.id where cast(i.created_at as Date)='$date' GROUP by i.created_at,i.user_id,u.own_id,u.name ");
